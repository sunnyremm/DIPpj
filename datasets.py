...
def make_dataset_mine(root: str):
    img_list = []
    for img_name in os.listdir(os.path.join(root, 'hazy')):
        img_list.append(os.path.join(root, 'hazy', img_name))
    return img_list


def make_dataset_HazeRD(root: str, mode: str):
    img_list = []
    for img_name in os.listdir(os.path.join(root, mode, 'simu')):
        name, ext = os.path.splitext(img_name)
        base_name = name.rsplit('_', 1)[0]
        gt_name = f"{base_name}_RGB{ext}"
        assert os.path.exists(os.path.join(root, mode, 'img', gt_name))
        img_list.append([os.path.join(root, mode, 'simu', img_name),
                         os.path.join(root, mode, 'img', gt_name)])
    return img_list
...
class MineDataset(data.Dataset):
    def __init__(self, root, mode=None):
        self.root = root
        self.mode = mode
        self.imgs = make_dataset_mine(root)
