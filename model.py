...
class DM2FNet_Norm(Base):
    def __init__(self, num_features=128, arch='resnext101_32x8d'):
        super(DM2FNet_Norm, self).__init__()
        self.num_features = num_features
        ...
        self.down1 = nn.Sequential(
            nn.Conv2d(256, num_features, kernel_size=1), nn.BatchNorm2d(num_features), nn.SELU() ##
        )
        self.down2 = nn.Sequential(
            nn.Conv2d(512, num_features, kernel_size=1), nn.BatchNorm2d(num_features), nn.SELU() ##
        )
        self.down3 = nn.Sequential(
            nn.Conv2d(1024, num_features, kernel_size=1), nn.BatchNorm2d(num_features), nn.SELU() ##
        )
        self.down4 = nn.Sequential(
            nn.Conv2d(2048, num_features, kernel_size=1), nn.BatchNorm2d(num_features), nn.SELU() ##
        )
        ...
        self.attention_phy = nn.Sequential(
            nn.Conv2d(num_features * 4, num_features, kernel_size=3, padding=1), nn.BatchNorm2d(num_features), nn.SELU(), ## 
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.BatchNorm2d(num_features), nn.SELU(), ##
            nn.Conv2d(num_features, num_features * 4, kernel_size=1)
        )
